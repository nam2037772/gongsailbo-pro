...
function parseExcelData(rows) {
  ...
  // 날짜 파싱 (H3 또는 G3 부근)
  let rawDate = rows[2] ? (rows[2][7] || rows[2][6] || '') : '';
  ...
  // 날씨 파싱 (H4 또는 G4)
  weatherStr = String(rows[3] ? (rows[3][7] || rows[3][6] || '') : '').trim();
  if (weatherStr === '주간공정율' || weatherStr === '') {
     weatherStr = String(rows[3] ? (rows[3][6] || rows[3][5] || '') : '').trim();
  }

  // 작업내용 및 특이사항 (사용자가 제공한 양식들의 표 범위 탐색)
  for (let r = 0; r < rows.length; r++) {
    ...
    for (let c = 0; c < row.length; c++) {
      ...
      if (cell === '금일작업내용' && !todayWorkStr) {
         todayWorkStr = String(rows[r+1] ? (rows[r+1][c] || rows[r+1][c-1] || '') : '').trim();
      }
      if (cell === '명일작업내용' && !tomorrowWorkStr) {
         tomorrowWorkStr = String(rows[r+1] ? (rows[r+1][c] || rows[r+1][c-1] || '') : '').trim();
      }
      if (cell === '특이사항' && !memoStr) {
         memoStr = String(rows[r+1] ? (rows[r+1][c] || rows[r+1][c-1] || '') : '').trim();
      }
    }
  }
  ...
  if (laborStartRow !== -1) {
    for (let r = laborStartRow; r < rows.length; r++) {
      ...
      // 오라통신공사협회 양식의 금일작업내용이 인원표 우측(col+5)에 병합되어 있는 경우
      if (row[laborCol + 5] && String(row[laborCol + 5]).trim() && String(row[laborCol + 5]).replace(/\s+/g, '') !== '명일작업내용' && String(row[laborCol + 5]).replace(/\s+/g, '') !== '특이사항') {
  ...
  // 핵심 문제 해결: render() 전에 기존에 남아있는 calcCache를 비워주어야 새 데이터가 화면에 출력됩니다.
  if (typeof clearCalcCache === 'function') clearCalcCache();
